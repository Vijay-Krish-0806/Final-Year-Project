...
export class GeminiLessonGenerator {
  async generateAssessmentLesson(language: string): Promise<GeneratedLesson> {
    const prompt = `
Create an assessment lesson for ${language} language learning with exactly 15 challenges.
      ...
      "options": [
        {
          "text": "Option text",
          "correct": true/false,
          "imageSrc": "/path/to/image.svg" (optional),
...
Make sure to have exactly 10 challenges with varied difficulty levels.
...
`;
  ...
  async analyzeAssessmentResults(
    userId: string,
    courseId: number,
    results: { challengeId: number; correct: boolean; topic: string }[]
...
Each unit should have 3-5 lessons, and each lesson should have 5-8 challenges.
